$(document).ready(() => {
  cometix.load(loadExtension);

  async function loadExtension(activationOpts = {}) {
    const $html = $('html');
    const $document = $(document);
    const $dom = $(TEMPLATE);
    const $sidebar = $dom.find('.cometix-sidebar');
    const $toggler = $sidebar.find('.cometix-toggle').hide();
    const $views = $sidebar.find('.cometix-view');
    const $spinner = $sidebar.find('.cometix-spin');
    const $pinner = $sidebar.find('.cometix-pin');
    ...
    $document
      .on(EVENT.REQ_START, () => $spinner.addClass('cometix-spin--loading'))
      .on(EVENT.REQ_END, () => $spinner.removeClass('cometix-spin--loading'))
    ...
    async function optionsChanged(event, changes) {
      ...
      if (await cometix.applyOptions(changes)) {
        reload = true;
      }
    ...
    async function tryLoadRepo(reload) {
      const token = await cometix.getAccessToken();
