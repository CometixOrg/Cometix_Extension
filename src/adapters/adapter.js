class Adapter {
      ...
      this._getSubmodules(tree, opts, (err, submodules) => {
        ...
        const nextChunk = async (iteration = 0) => {
          ...
          for (let i = 0; i < CHUNK_SIZE; i++) {
            ...
            await cometix.setNodeIconAndText(this, item);

            if (item.patch) {
              item.text += `<span class="cometix-patch">${this.buildPatchHtml(item)}</span>`;
            }
  ...
  async _handleError(settings, jqXHR, cb) {
    ...
    switch (jqXHR.status) {
      ...
      case 401:
        error = 'Invalid token';
        message = await cometix.getInvalidTokenMessage({
      ...
      case 403:
        if (jqXHR.getResponseHeader('X-RateLimit-Remaining') === '0') {
          ...
          message =
            ...
            'To continue using cometix, you need to provide a GitHub access token. ' +
            'Please go to <a class="settings-btn">Settings</a> and enter a token.';
        } else {
          error = 'Forbidden';
          message =
  ...
  /**
   * Returns the CSS class to be added to the cometix sidebar.
   * @api public
   */
  getCssClass() {
  ...
  buildPatchHtml(treeItem = {}) {
    ...
    patch += files ? `<span class='cometix-patch-files'>${files} ${files === 1 ? 'file' : 'files'}</span>` : '';
