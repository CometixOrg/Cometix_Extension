class OptionsView {
  constructor($dom, adapter) {
    ...
    this.$toggler = $dom.find('.cometix-settings').click(this.toggle.bind(this));
    this.$view = $dom.find('.cometix-settings-view').submit((event) => {
      event.preventDefault();
      this.toggle(false);
    });

    this.$view.find('a.cometix-create-token').attr('href', this.adapter.getCreateTokenUrl());
